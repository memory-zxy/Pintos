...
process_execute (const char *file_name) 
{
  char *fn_copy;
  tid_t tid;
  char *ex_type = malloc(strlen(file_name)+1);
  char *save_ptr;
  ...
  strlcpy (ex_type, file_name, strlen(file_name)+1);
  ex_type = strtok_r(ex_type, " ", &save_ptr);
  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (ex_type, PRI_DEFAULT, start_process, fn_copy);  
  free(ex_type);
  if (tid == TID_ERROR)
    palloc_free_page (fn_copy); 
  sema_down(&thread_current()->child_lock); // wait for child
  if (!thread_current()->success) return -1; // for task multi-oom
  return tid;
}
...
start_process (void *file_name_)
{
  ...
  if (t->parent->cwd) t->cwd = dir_reopen(t->parent->cwd);
  else t->cwd = dir_open_root();
...
{
  struct list_elem *e;
  struct list_elem *check_e = NULL;
  struct child *c;
  if (list_empty(&thread_current()->children)) return -1;
  // find the child
  ...
  if (!check_e || !c) return -1;
  thread_current()->wait_tid = c->tid;
  if (!c->used) {
...
process_exit (void)
{
  ...
  if (cur->exit_code == -100) exit_proc(-1); // without SYS_EXIT
  pd = cur->pagedir;
  // acquire_filesys_lock();
  file_close(cur->exec_file); // re-enable write
  struct list_elem *e;
  ...
  while (!list_empty(mmlist)) {
    ...
    ASSERT( sys_munmap (desc->mid) == true );
  }
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      if (! page_table_install_filesys(curr->page_table, upage,
...
install_page (void *upage, void *kpage, bool writable)
{
  ...
  /* Verify that there's not already a page at that virtual
     address, page_tablehen map our page there. */
  bool success = (pagedir_get_page (t->pagedir, upage) == NULL);
  success = success && pagedir_set_page (t->pagedir, upage, kpage, writable);
#ifdef VM
  success = success && page_table_install_frame (t->page_table, upage, kpage);
